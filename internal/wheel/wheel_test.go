...
func TestDelayScheduleMonotonicallyIncreases(t *testing.T) {
	configs := []struct {
		name  string
		v0    float64
		alpha float64
	...
	for _, c := range configs {
		t.Run(c.name, func(t *testing.T) {
			delays := delaySchedule(c.v0, c.alpha, 1.0)
			if len(delays) == 0 {
...
func TestDelayScheduleTickCount(t *testing.T) {
	// With v0=35, alpha=20, maxDelay=0.5s the expected tick count is 30:
	// full traversals where v0^2 - 2*alpha*(k+1) >= 0, i.e. k < v0^2/(2*alpha) - 1
	// floor((35^2)/(2*20) - 1) + 1 = floor(29.625) + 1 = 30.
	delays := delaySchedule(35.0, 20.0, 0.5)
	if len(delays) != 30 {
		t.Errorf("delaySchedule(35,20,0.5) produced %d ticks, want 30", len(delays))
	}
}

func TestDelayScheduleFirstDelayMatchesV0(t *testing.T) {
	v0 := 35.0
	delays := delaySchedule(v0, 20.0, 0.5)
	if len(delays) == 0 {
		t.Fatal("no delays")
	}
	expected := 1.0 / v0
...
func TestDelayScheduleLastDelayBelowMax(t *testing.T) {
	maxDelay := 0.5
	delays := delaySchedule(35.0, 20.0, maxDelay)
	if len(delays) == 0 {
		t.Fatal("no delays")
	}
	last := delays[len(delays)-1].Seconds()
	if last > maxDelay {
		t.Errorf("last delay = %v s exceeds maxDelay = %v s", last, maxDelay)
	}
}

func TestDelaySchedulePhysicsFormula(t *testing.T) {
	v0, alpha := 35.0, 20.0
	delays := delaySchedule(v0, alpha, 1.0)
	v0sq := v0 * v0
...
func TestDelayScheduleHighFrictionFewTicks(t *testing.T) {
	low := delaySchedule(35.0, 10.0, 0.5)
	high := delaySchedule(35.0, 50.0, 0.5)
	if len(high) >= len(low) {
		t.Errorf("high friction produced %d ticks >= low friction %d ticks", len(high), len(low))
	}
}

func TestDelayScheduleHighForceMoreTicks(t *testing.T) {
	slow := delaySchedule(20.0, 20.0, 0.5)
	fast := delaySchedule(50.0, 20.0, 0.5)
	if len(fast) <= len(slow) {
		t.Errorf("higher v0 produced %d ticks <= lower v0 %d ticks", len(fast), len(slow))
	}
}
...
func TestSpinExplicitStart(t *testing.T) {
	...
	cfg := defaultTestCfg()
	cfg.Start = 2 // 0-indexed → items[2] = "c"
...
func TestSpinValidation(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:    "empty items",
			items:   nil,
			cfg:     Config{Force: 1, Mass: 1, Friction: 0.2, MaxDelay: time.Second},
			wantErr: "no items",
		},
		{
			name:    "zero force",
			items:   []string{"a"},
			cfg:     Config{Force: 0, Mass: 1, Friction: 0.2, MaxDelay: time.Second},
		...
		},
		{
			name:    "zero friction",
			items:   []string{"a"},
			cfg:     Config{Force: 1, Mass: 1, Friction: 0, MaxDelay: time.Second},
			wantErr: "friction must be positive",
		},
		{
			name:    "zero max delay",
			items:   []string{"a"},
			cfg:     Config{Force: 1, Mass: 1, Friction: 0.2, MaxDelay: 0},
